...
const asString = (value: string | boolean | undefined, fallback?: string): string | undefined => {
...
const requireAgentId = (agentId: string | undefined): string => {
  if (!agentId) {
...
const run = async () => {
  ...
  const env = getEnv();
  const client = new AgentClient({ serverUrl: env.serverUrl, apiKey: env.apiKey });
  ...
  switch (command) {
    ...
    case 'log': {
      const agentId = requireAgentId(env.agentId);
      const level = asString(parsed.flags.level, 'info') as string;
    ...
    case 'file': {
      ...
      const mime = asString(parsed.flags.mime, 'application/octet-stream') as string;
    ...
    case 'heartbeat': {
      const agentId = requireAgentId(env.agentId);
      const status = asString(parsed.flags.status, 'online') as string;
